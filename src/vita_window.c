...
static int resizeNativeWindow(_GLFWwindow* window, int width, int height)
{
    ...
    window->vita.width = width;
    window->vita.height = height;
...
{
    _glfwInputError(GLFW_API_UNAVAILABLE, "Vita: EGL library not loaded");
    return GLFW_FALSE;
}
                              ...
                              const _GLFWfbconfig* fbconfig)
{
    ...
    {
        _glfwInputError(GLFW_PLATFORM_ERROR, "Vita: Cannot create more than one window");
        return GLFW_FALSE;
    }
    ...
    if (ctxconfig->client != GLFW_NO_API)
    {
        ...
        {
            _glfwInputError(GLFW_API_UNAVAILABLE, "Vita: OSMesa not available");
            return GLFW_FALSE;
        }
    }

    _glfw.vita.cur_window = window;
    return GLFW_TRUE;
}
...
{
    if (width)
        *width = window->vita.width;
    if (height)
        *height = window->vita.height;
}
